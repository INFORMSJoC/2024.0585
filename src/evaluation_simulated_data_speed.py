...
def get_all_candidate_list(transaction_dict):
    begin = dt.datetime.now()
    result = []
    print(begin, 'start get all candidates')
    item_frequency_dict = adlt.get_item_frequency_dict(transaction_dict)
    item_list = list(item_frequency_dict.keys())
    if '' in item_list:
        item_list.remove('')
    if 'total_number_of_record' in item_list:
        item_list.remove('total_number_of_record')
    for i in range(len(item_list)-1):
        for j in range(i, len(item_list)):
            if item_list[i][0] != item_list[j][0]:
                if item_list[i][0] == 'd':
                    result.append([item_list[i], item_list[j]])
                else:
                    result.append([item_list[j], item_list[i]])
    end = dt.datetime.now()
...
def get_second_step_result(first_step_result, transaction_dict, correlation_type, correlation_threshold, cc=0.5, whether_correct=True, target_p_value=0.05, delta=0.0001, whether_speed_up_screen=True):
    ...
    for each_pair in first_step_result:
        ...
        if (correlation_estimation > no_correlation_value + 0.0000000001) | (correlation_estimation < no_correlation_value - 0.0000000001):
            non_speed_up_count = non_speed_up_count + 1
        else:
            speed_up_count = speed_up_count + 1
        if correlation_estimation > correlation_threshold:
...
def speed_test():
    ...
    for para_list in test_setting_list:
        ...
        # start the check for all step
        target_p_value = 0.005
        first_result = branch_range_search(test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value, whether_general_half_search=False, whether_relaxed_upperbound=False)
        second_result = get_second_step_result(first_result, test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value)
        print()
        first_result = branch_range_search(test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value, whether_general_half_search=False, whether_relaxed_upperbound=True)
        second_result = get_second_step_result(first_result, test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value)
        print()
        first_result = branch_individual_search(test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value, whether_relaxed_upperbound=True)
        second_result = get_second_step_result(first_result, test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value)
        print()
        first_result = get_all_candidate_list(test_tran_dict)
        second_result = get_second_step_result(first_result, test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value)
        print()
        first_result = get_all_candidate_list(test_tran_dict)
        second_result = get_second_step_result(first_result, test_tran_dict, correlation_type, correlation_threshold, target_p_value=target_p_value, whether_speed_up_screen=False)
